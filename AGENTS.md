...
dotnet build
dotnet test
dotnet test --filter "FullyQualifiedName!~Integration&Category!=Aspire"   # fast: unit + convention + fuzz
...
| Command/query handlers | `.agents/skills/cqrs-patterns/SKILL.md`, pinned exemplars in `docs/exemplars/` |
| Domain models, value objects | `.agents/skills/ddd-implementation/SKILL.md` |
| EF Core config, migrations, Aspire wiring | `.agents/skills/data-access/SKILL.md` |
| Minimal API endpoints | `.agents/skills/api-design/SKILL.md` |
...
`AGENTS.md`/`.agents/skills` and `CLAUDE.md`/`.claude/skills` are mirrors; `AgentDocsConventionTests` fails the build on any drift beyond the doc-name and skills-path tokens. Edit both sides in the same change.
