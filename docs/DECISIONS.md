...
- Claims map to `ICurrentUser` in exactly one place (the identity infrastructure in `Api/Infrastructure/Identity/`): `sub` → Subject, `tid` → TenantId, `scp`/`scope` → Scopes, `amr` → AuthenticationMethods. Production code never reads `HttpContext.User`, raw claims, or the `Authorization` header outside that namespace — convention-enforced.
