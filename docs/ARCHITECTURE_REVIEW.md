...
>
> **Open after this session** (see Open Findings below): create-order retry-idempotency has no regression test; gateway-assertion replay/nonce and body/query signing are accepted threat-model decisions; an `aspire` CI flake (slow Service Bus emulator startup → health-check timeout) is a watch-item, not a defect.
>
...
> **CI build break fixed 2026-06-09 (post-supply-chain).** `main` CI was red from two **independent** failures (not the supply-chain work); both fixed and verified full-pipeline green.
> - **Unit-test job — Docker Hub pull timeout.** 55 DB-backed command/query handler tests failed with `context deadline exceeded` pulling `postgres:16-alpine`. These tests share a Testcontainers PostgreSQL fixture even though they are **not** named `*Integration*`, so the unit step needs the image too. The cause was anonymous Docker Hub throttling on shared GitHub Actions IPs against Testcontainers' short, non-retrying pull deadline. Fix: a 5-attempt backoff loop pre-pulls the image to warm Docker's local cache before the test steps in the `build` and `integration` jobs (the fixture then resolves it locally and never hits the registry). No secrets. Commit `9b25cfa`.
> - **Aspire job — minute-bucket race in `CreateCustomer_ShouldArchivePayloadsToAspireBlobStorage`.** The test assumed a single request's inbound/outbound captures share one archive blob and that the first-found archive blob is the one referenced by the audit/entity-index pointers. Both break near a minute rollover: archive paths are bucketed `archive/{date}/{HH}/{mm}/{correlationId}.jsonl`, so the two directions can split across two blobs and a pointer can name a later bucket than the one enumerated first — which raced under slow CI emulator startup. Fix: assert archive content over the **union** of the correlation's archive blobs (`PollForBlobContentsAsync`), and validate audit/entity-index pointers by **shape** (`archive/.../{correlationId}.jsonl`) scanning **per JSONL row** for the correlation's own row — necessary because audit blobs are minute-window files shared across correlations, and uncorrelated `/health/ready` probes write their own GUID-correlation rows (an early fix attempt that grabbed the first pointer in the file matched a health-probe row and failed). Commit `14fc72a`. Both fixes verified: run `27173837911` green across build/integration/aspire/docker-build.
> - Score **unchanged (8.2)** — CI-robustness fixes (a test assumption and a registry-flake guard), not a runtime defect or a capability that changes the template's altitude.
...
The 2026-06-07 batch (**#74, #75, #76, #77, #78, #79, #81**) is resolved — see the table immediately below; **#80** is an accepted limitation (EF cannot express the index). The 2026-06-09 hardening session (see the dated note above) closed the build/CI + invariant/test cluster from the independent re-review. The following remain **open**:

- **Create-order retry-idempotency is untested.** `CreateOrderCommandHandler` generates one stable order Id outside the EF execution-strategy retry delegate so a commit-unknown retry cannot create a second order / double-reserve stock. The invariant holds in code but has no regression test (faithfully testing it needs a `DbCommandInterceptor` injecting a post-commit transient fault). Low severity; documented gap.
- **Gateway-assertion replay window (decision, not a bug).** No jti/nonce/consumed-token store; replay is bounded only by the ~150s lifetime+skew window. Acceptable under the trusted-gateway+TLS threat model; a Redis-backed nonce cache is the cheap upgrade if a higher-assurance deployment needs it.
- **Gateway-assertion does not sign request body/query (decision, not a bug).** The signature binds method/path/identity-hash/scopes but not the body or query string, so within the lifetime window a body could be altered. Acceptable under the same threat model; harden or document per deployment.
