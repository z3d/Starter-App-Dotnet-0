...
No open findings remain. **#74, #75, #76, #77, #78, #79, #81** are resolved — see the table immediately below. **#80** was investigated and reclassified as an **accepted limitation** (EF cannot express the index; see the limitation note below the table).
...
**Accepted limitation (residual test gap, 2026-06-08) — legacy owner-stamping constructors are public but unguarded by a convention test.** `Order`/`Customer`/`Product` expose convenience constructors that omit owner scope and stamp `OwnershipDefaults.Legacy*` sentinels; they are a back-compat/test-ergonomics shim used only by tests (~87 sites) — all 3 production construction sites in `StarterApp.Api` use the owner-aware overloads (verified by grep). A convention test to bar legacy-ctor use in production was attempted but removed: IL `newobj` scanning is config-dependent (passed in Debug, found zero constructions in Release), and the compile-time fix (making the ctors `internal`) is blocked because `Domain` already grants `InternalsVisibleTo("StarterApp.Api")`. The robust enforcement path is to delete the legacy overloads and route tests through the owner-aware ctor / a test builder, removing the footgun entirely (deferred — ~87 call-site refactor). Until then the residual risk is a *future* production handler calling a no-owner ctor; existing production code is correct. Recorded per CLAUDE.md's "document the residual test gap" rule.
