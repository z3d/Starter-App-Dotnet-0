...
- **Folder-only Clean Architecture (explained, user-deferred).** `Domain` is its own assembly (purity compiler-enforced), but `Application` and `Infrastructure` are folders inside `StarterApp.Api`, so that boundary is convention-enforced only. This is the one place "be a complete reference" argues for *more* structure (splitting Application/Infrastructure into separate projects so the dependency rule is compiler-enforced) rather than accepting the lighter form. Deferred as a multi-project refactor decision; not a defect.
