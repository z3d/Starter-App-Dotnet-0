...
| U2 | Medium | Correlation-id canonicalization mismatch: `GatewayIdentityHeaders.Read` stored `CorrelationContext.Sanitize(correlationId)` and hashed the sanitized value, but the external gateway signs the **raw** id — so any id with `:`/`/`/space/`+`/`=` or >128 chars failed both the `cid` and `HeaderHash` checks and rejected a validly-signed request 401 under `Mode=Required`. | Made the correlation id a contract-bound token: `Read` validates the raw value against `[A-Za-z0-9._-]{1,128}` (rejects out-of-charset/over-length up front) and stores it **verbatim** so signer and verifier canonicalize identically; `Sanitize` is a no-op on already-valid values. Tests: charset/length rejection theory + raw-value-parity case in `GatewayIdentityHeadersTests`. Contract documented in CLAUDE.md/AGENTS.md. |
