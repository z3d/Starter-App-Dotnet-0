...
**Score: 9.5/10** (80 findings resolved; 4 open low-severity findings from the 2026-06-07 review)
...
Still open after this session: **#76, #78, #79, #80, #81** (all low / opinionated). **#74, #75, #77** are resolved — see the table immediately below.
...
|---|----------|---------|---------------|
| 76 | Medium | **Money/GST math is never rounded to a currency scale.** `OrderItem.GetUnitPriceIncludingGst/GetTotalGstAmount/...` compute `Amount * GstRate` ([OrderItem.cs:48-73](../src/StarterApp.Domain/Entities/OrderItem.cs)) and `Money.Create` ([Money.cs:16-26](../src/StarterApp.Domain/ValueObjects/Money.cs)) does not quantize. A valid `9.99 × 0.10` yields `0.999`; a `DECIMAL(5,4)` rate like `0.0750` yields `0.74925`. These sub-cent values flow into `OrderDto` totals and the persisted/published `OrderCreatedDomainEvent` totals. (Write/read paths agree algebraically — there is no CQRS divergence — but both emit non-money-shaped decimals.) | Quantize in `Money.Create` (or a `Money.Round`) to 2 dp with a documented rounding mode, and pick a single canonical GST rounding point (recommend: round GST per line item then sum). Add a property test asserting every `Money` produced by order math has scale ≤ 2. |
| 78 | Low | **`FailClosed` is silently bypassed by the null archive store.** `PayloadCaptureSink.CaptureAsync` returns when `_store is NullPayloadArchiveStore` ([PayloadCaptureSink.cs:40-49](../src/StarterApp.ServiceDefaults/Payloads/PayloadCaptureSink.cs)) **before** `FailureMode` is consulted (FailureMode only gates the `catch` at line 127). So `FailureMode=FailClosed` with no store fails *open* and drops payloads. Production is protected by the independent `RequireArchiveStore=true` startup validation, so this only bites when the two settings disagree (misconfiguration). | In the null-store branch, throw when `FailureMode==FailClosed`; only fail open under `FailOpen`. Or imply `RequireArchiveStore` whenever `FailClosed`. |
| 79 | Low | **A non-transient payload-capture failure permanently poisons outbox messages.** Capture runs before the Service Bus send ([OutboxProcessor.cs:88-104](../src/StarterApp.Api/Infrastructure/Outbox/OutboxProcessor.cs)). Under `FailClosed`, a persistent *non-transient* capture fault (e.g. Blob 403, config/serialization defect) is not matched by `IsTransientDependencyError`, so it falls to `IncrementRetry()` → after `MaxRetries` → `MarkAsError`: the event is permanently errored and never published though Service Bus was healthy, requiring manual DB requeue. The transient path correctly pauses the batch; the non-transient path loses events. | On capture failure (as opposed to publish failure), break/pause the batch rather than `IncrementRetry`, or capture after a successful send. Distinguish capture-side from publish-side failures. |
| 80 | Low | **Unique customer-email index exists in SQL but not in the EF model.** The baseline declares `ix_customers_tenant_id_owner_subject_email` UNIQUE ([0001_CreatePostgresSchema.sql:29-30](../src/StarterApp.DbMigrator/Scripts/0001_CreatePostgresSchema.sql)), but `CustomerConfiguration` declares only the non-unique `ix_customers_tenant_id_owner_subject`. The handlers rely on catching that exact constraint name as the race-safe uniqueness net; production is fine (DbUp is the schema source of truth) but the EF model is not authoritative — it only bites under `EnsureCreated()`/model-diff. | Add `HasIndex(c => new { c.TenantId, c.OwnerSubject, c.Email }).IsUnique().HasDatabaseName("ix_customers_tenant_id_owner_subject_email")`. |
| 81 | Low | **`DapperConventionTests.QueryHandlers_MustNotUseSelectStar` lacks an `Assert.NotEmpty` guard** ([DapperConventionTests.cs:11](../src/StarterApp.Tests/Conventions/DapperConventionTests.cs)). Best.Conventional's `MustConformTo` passes vacuously over an empty set, so if the `QueryHandler` suffix convention ever changed the test would silently go green. Its sibling `MustUsePostgresRetryPolicy` guards with `Assert.NotEmpty`; this one is the odd one out. | Add `Assert.NotEmpty(queryHandlers.ToList())` before `MustConformTo`. |
