...
**Accepted limitation (2026-06-10) — Product/Customer create retry-idempotency under commit ambiguity.** `CreateCustomerCommandHandler` is now idempotent across execution-strategy retries via its natural key (owner-scoped unique email): a retry whose prior commit actually succeeded finds and returns the committed row instead of throwing a spurious duplicate-email 409. `CreateProductCommandHandler` has **no natural key and a DB-generated int id**, so a commit-succeeded-but-ack-lost retry can still insert a silent duplicate product row. The real fix is a client-generated `Guid.CreateVersion7()` id (the CreateOrder pattern) or a unique business key — both are schema/contract changes deliberately left as a stakeholder decision (the aggregate-ID convention explicitly permits DB-generated ids for aggregates without creation events). Re-run update/delete commands producing false `xmin` 409s are inherent to optimistic concurrency and accepted (visible, safe, no data loss). **Residual test gaps (documented per the review-fix rule):** the customer recheck path cannot be driven on the InMemory provider (no execution-strategy retries) — covering it needs a fault-injecting interceptor test like `CreateOrderConcurrencyIntegrationTests`; the oversize-sidecar write path is unit-tested at the naming/pointer level but not exercised against real Azure blob concurrency; the cleanup TimerTrigger `:`-form fix is convention-tested but worth one observation on a real Functions host.

**Accepted limitation (2026-06-10) — audit/archive blob trail is not WORM-protected.** The payload archive/audit/entity-index blobs are the system's compliance evidence, but nothing prevents their deletion inside the retention window: any credential with blob-delete rights (including the cleanup Function, or a bug in it) can destroy records before `PayloadCapture:RetentionDays` elapses. A time-based immutability (WORM) policy on the `audit/` (and optionally `archive/`) containers would close this, but it is a storage-resource configuration this repo cannot express: local orchestration runs the storage emulator, which does not enforce immutability policies, and the repo deliberately ships no production IaC (deployment topology is owned by the hosting environment — maintainer decision 2026-06-10). **Action for deployers:** apply a time-based retention/immutability policy aligned with `PayloadCapture:RetentionDays` to the audit containers in production-like environments; the cleanup Function then only ever succeeds on blobs already past the policy window. **Revisit if:** the template gains a production provisioning path, or the emulator/orchestration stack gains a way to express container immutability that could be convention-tested.
...
All nine confirmed findings fixed with regression tests; full build clean (0 warnings, warnings-as-errors) and the StarterApp.Tests suite green (591 tests).

| # | Severity | Finding | Fix |
|---|----------|---------|-----|
| U1 | Medium (latent) | Gateway-security convention tests (`ApiRouteEndpoints_MustRequireGatewayIdentity`/`_MustRequireGatewayScope`/`ApiWriteEndpoints_MustBeSecuredBy2Fa`) asserted only `failures.Count == 0` with no non-empty guard, so a marker/namespace/route-prefix refactor that yielded zero `/api/v1` routes would pass all three vacuously while the protected surface went unverified for identity/scope/MFA. | Added `Assert.NotEmpty(endpoints)` in the shared `GetApiRouteEndpoints` choke point (hardens all three at once). Proven to fail with "Collection was empty" when the route filter is pointed at a non-existent prefix, then reverted. |
| U2 | Medium | Correlation-id canonicalization mismatch: `GatewayIdentityHeaders.Read` stored `CorrelationContext.Sanitize(correlationId)` and hashed the sanitized value, but the external gateway signs the **raw** id — so any id with `:`/`/`/space/`+`/`=` or >128 chars failed both the `cid` and `HeaderHash` checks and rejected a validly-signed request 401 under `Mode=Required`. | Made the correlation id a contract-bound token (`[A-Za-z0-9._-]{1,128}`): `Read` validates the raw value (rejects out-of-charset/over-length up front) and stores it **verbatim** so signer and verifier canonicalize identically. **Completed 2026-06-09 (follow-up):** the initial `Read`-only fix was inert because `PayloadCaptureMiddleware` (runs before `UseGatewayIdentity`) rewrote the request `X-Correlation-ID` to a sanitized value before `Read` saw it — so `Read` never received a raw value to validate. The middleware now leaves a *present* correlation id on the request unchanged (only injecting a generated one when the caller sends none), so the gateway layer validates the exact value the gateway signed; the echoed/archived id stays sanitized (no raw client input reflected downstream). Tests: charset/length rejection theory in `GatewayIdentityHeadersTests` (now reachable in the pipeline) + pass-through / echo-sanitized / inject-when-absent cases in `PayloadCaptureMiddlewareTests`. Contract documented in CLAUDE.md/AGENTS.md. |
| U3 | Medium | Unbounded entity-index fan-out: `PayloadEntityReferenceExtractor` added one reference per `id`/`*Id` property with no count cap, and the sink writes one serial Azure blob (~3 round trips) per reference inline on the request thread (middleware runs pre-auth/pre-rate-limit), so a default-size body packed with distinct `*Id` names amplified one request into thousands of storage calls. | Capped the returned reference count via new `PayloadCaptureOptions.MaxEntityReferences` (default 64); the sink records `entityReferencesTruncated=true` in archive metadata when the cap trips so support can distinguish bounded captures. Test: 10k-distinct-`*Id` body caps at 64 + flags truncation. |
| U4 | Low | `page` int32 overflow: paged query validators bounded `pageSize` but not `page`, so `(page-1)*pageSize` (unchecked `int*int`) wrapped to a negative value → PostgreSQL rejects the negative `OFFSET` → unmapped `PostgresException` → 500 on every list endpoint. | Added an upper bound (`Page > 100_000 → ValidationError`) to all four paged validators (100000×100 = 1e7, far under `int.MaxValue`). Test: `PagedQueryValidatorTests` asserts each validator flags `page=200000` and accepts `page=100000`. |
| U5 | Low | Order list pagination ordered by non-unique `order_date` with `LIMIT/OFFSET`, so same-timestamp tie groups straddling a page boundary could duplicate or skip rows across pages. | Appended the unique pk tie-breaker `, o.id DESC` to `GetOrdersByStatusQuery`/`GetOrdersByCustomerQuery` (no schema change). Integration test forces identical `order_date` across 5 orders and asserts paging returns each exactly once. |
| U6 | Low | Post-commit `Invalidate*Async` ran un-try/catch'd across all 7 mutation handlers, so a transient Redis outage turned an already-committed write into a 500 and left the entry stale. | Wrapped invalidation in `CacheInvalidator` (single choke point) in a best-effort `try/catch` that logs and swallows non-cancellation cache errors — caching is a best-effort sidecar and must not fail a committed write. Test: throwing `IDistributedCache` double → `InvalidateProductAsync` does not propagate. |
| U7 | Low | Read-through cache repopulation race: a read that missed before a concurrent write could `SetString` the pre-write value after the write's `Remove`, serving stale data for the full TTL (partially defeated the #74 invalidation guarantee). | Added a short-TTL invalidation tombstone (`{key}:inv`, 10 s): `CacheInvalidator` writes it on invalidation; `CachingBehavior` checks it immediately before repopulating and skips the write if present. Test: tombstone-present → no repopulation. (Display-scoped; order placement still re-validates stock against the live DB + `xmin`.) |
| U8 | Medium | Migration-safety convention tests (`MigrationScripts_MustNameAllConstraintsExplicitly` + the two embedded-script tests) shared `if (scriptsDir == null) return;` with zero assertions, so a path/layout drift made the constraint-naming guard validate nothing — same vacuous-pass class as U1. | Routed all three through a new `RequireScriptsDirectory()` that `Assert.NotNull(scriptsDir)` + `Assert.NotEmpty(*.sql)`, so an unresolvable/empty Scripts dir fails loudly instead of passing silently. |
| U9 | Low | `AzureBlobPayloadArchiveStore.AppendLineAsync` wrote the whole serialized line as one `AppendBlock`; `MaxPayloadBytes` `[Range(1,100 MiB)]` permits lines above Azure's 4 MiB block ceiling → `RequestFailedException` (batch pause under SB `FailClosed`, silent audit loss under HTTP `FailOpen`). | Split oversize lines into ≤4 MiB ordered `AppendBlock` calls (Append Blob preserves order) via a pure, unit-tested `ChunkBlocks` helper. Tests: single-chunk under limit; >4 MiB splits into ordered chunks none over limit whose concatenation reproduces the input. |

The single dismissed candidate (a stale Dockerfile lock-restore comment) was refuted by verification and is not actioned.
