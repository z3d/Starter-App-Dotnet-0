...
Read it before starting (it carries the open findings and current score; the dated history and dismissed false positives live in docs/reviews/) and update it after: mark findings resolved with the fix + the regression test added, record dismissed false positives so they aren't re-raised, and adjust the score conservatively. It is the sync point across concurrent agent sessions. Keep AGENTS.md/CLAUDE.md and `.agents/skills`↔`.claude/skills` in sync per the repo's drift rule.
