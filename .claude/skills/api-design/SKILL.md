...
- Every `/api/v1` group calls `.RequireGatewayIdentity()`.
- Every route declares `.RequireScope("{area}:read|write")`.
- Every non-GET route also calls `.SecuredBy2Fa()` (gateway-signed `amr` must include `mfa`).
- Every handler binds a `CancellationToken` and forwards it to `mediator.SendAsync`.
- Health/liveness probes stay anonymous — orchestrator probes send no identity headers.
