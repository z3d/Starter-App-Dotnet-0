...
- **`docs/_local/consistency-*.txt`** if present (git-ignored, regenerated by every test run) —
  the advisory cohort reports. Treat a two-sigma outlier or a per-feature divergence on a changed
  handler or configuration as a prompt to look, never as a finding by itself.
